...
import com.example.doggysitter.adapters.AdminWalkerProfileAdapter;
...
public class AdminWalkerProfilesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        titleTextView.setText(R.string.walker_profiles);
        emptyTextView.setText(R.string.no_admin_walker_profiles);
    ...
    private void loadWalkerProfiles() {
        setLoading(true);
        adminRepository.getAllWalkerProfiles()
                .addOnSuccessListener(querySnapshot -> {
                    List<WalkerProfile> profiles = new ArrayList<>();
                    for (DocumentSnapshot documentSnapshot : querySnapshot.getDocuments()) {
                        WalkerProfile profile = documentSnapshot.toObject(WalkerProfile.class);
                        if (profile != null) {
                            if (TextUtils.isEmpty(ValidationUtils.normalizeSpaces(profile.getUid()))) {
